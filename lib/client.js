window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * True when the job actually failed. The wire reports "failed" only
		 * when the executor contract broke; a non-zero exit code lands as
		 * "completed" with detail like "exit code: 1" — parse that so a
		 * failing command alerts as a failure, not a success.
		 */
		function jobFailed(job) {
			if (job.status === "failed" || job.status === "killed") return true;
			if (job.status !== "completed" || !job.detail) return false;
			const match = /exit code:\s*(-?\d+)/.exec(job.detail);
		...
		function systemNotify(job, t) {
			...
			const bad = jobFailed(job);
			const short = shortLabel(job.label);
			const title = job.status === "killed"
				? t("notify.killed", { label: short })
				: bad ? t("notify.failed", { label: short })
				: t("notify.done", { label: short });
		...
		let flashTimer = null;
		let lastLiveCount = 0;
		/** Truncate very long job labels for the tab title / notification. */
		...
		function flashTitle(ok, label) {
			...
			document.title = (ok ? "✓ " : "✗ ") + shortLabel(label);
		...
		function refreshTitle(liveCount) {
			if (liveCount !== undefined) lastLiveCount = liveCount;
			if (Date.now() < flashUntil) return;
			if (baseTitle === null) baseTitle = document.title;
			document.title = lastLiveCount > 0 ? `(${lastLiveCount}) ${baseTitle}` : baseTitle;
		}
		//#endregion
		 ...
		 */
		function useCompletionAlerts(jobs, soundOn, notifyOn, t, onFlash) {
			const seen = useRef(new Map());
			useEffect(() => {
				const map = seen.current;
				for (const job of jobs) {
					const prev = map.get(job.id);
					// prev 是上一帧的状态字符串（如 "running"），直接用集合判活，
					// 不能调 isLive(prev) —— 它接收任务对象、访问 job.status。
					if (prev && LIVE_STATUSES.has(prev) && SETTLED_STATUSES.has(job.status)) {
						const ok = !jobFailed(job);
						if (soundOn) chime(ok);
						if (notifyOn) systemNotify(job, t);
						flashTitle(ok, job.label);
						onFlash(job.id, ok);
					}
					map.set(job.id, job.status);
				}
			}, [jobs, soundOn, notifyOn, t, onFlash]);
		}
		//#endregion
		...
		function AttentionPack({ sessionId, useSessions, t }) {
			...
			useCompletionAlerts(jobs, soundOn, notifyOn, t, onFlash);
				...
				h("div", { className: "dap_list" },
					rows.map((job) => {
						...
						return h("div", {
							...
							h("span", { className: "dap_dot " + (liveNow ? "running" : bad ? "failed" : job.status) }),
